...
export default function Standards() {
  ...
  const [relatedStandardsSearch, setRelatedStandardsSearch] = useState('');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const standardData: any = {
        ...
        validityMonths,
        nextReviewDate,
        relatedStandardIds: editingStandard.relatedStandardIds || [],
      };
      ...
      >
        {editingStandard && (
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="mt-1 block w-full rounded-lg border-gray-300 shadow-sm p-2.5 border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all outline-none text-sm bg-white"
            ...
            {/* Fila 2: Tipo de Relación y Tipo de Contenido */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Hierarchy Level Selection */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Relacionar con Jerarquía</label>
                <div className="flex gap-4 p-2.5 bg-gray-50 rounded-lg border border-gray-200">
            ...
            {/* Dos Columnas para Detalles Relacionados y Contenido/Fechas */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2 border-t border-gray-150">
              
              {/* Columna Izquierda: Listas de Relaciones (Checkbox selector) */}
              <div className="space-y-4">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Asociación en Jerarquía</h3>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-xs text-blue-600 hover:text-blue-850 font-semibold cursor-pointer"
                      >
                    ...
                    />
                    <div className="max-h-56 overflow-y-auto border border-gray-200 rounded-lg p-2.5 space-y-1.5 bg-gray-50 custom-scrollbar">
              ...
              <div className="space-y-4">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Detalles de Contenido y Validez</h3>
                ...
                {editingStandard.contentType === 'link' ? (
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Documento del Estándar</label>
                    <div className="flex items-center gap-3">
                      <button
                        type="button"
                        onClick={() => fileInputRef.current?.click()}
                        disabled={uploading}
                        className="flex items-center gap-2 px-4 py-2.5 bg-gray-100 border border-gray-300 rounded-lg text-sm text-gray-700 font-semibold hover:bg-gray-200 transition-colors cursor-pointer shrink-0"
                      >
                        {uploading ? (
                      ...
                      {editingStandard.fileUrl && (
                        <div className="flex items-center gap-1.5 text-emerald-600 text-sm font-medium bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-100 overflow-hidden">
                ...
                <div className="bg-blue-50 p-3.5 rounded-xl border border-blue-100 flex items-center justify-between text-sm shadow-sm">
                ...
                <div className="pt-4 border-t border-gray-150">
                  <label className="block text-sm font-medium text-gray-700 mb-1.5 font-semibold">Estándares Relacionados (Grafo)</label>
                  <input
                    type="text"
                    placeholder="Buscar estándar para relacionar..."
                    value={relatedStandardsSearch}
                    onChange={(e) => setRelatedStandardsSearch(e.target.value)}
                    className="w-full text-sm p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none mb-2 bg-white"
                  />
                  <div className="max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-2.5 space-y-1.5 bg-gray-50 custom-scrollbar">
                    {standards
                      .filter(s => s.id !== editingStandard.id && s.name.toLowerCase().includes(relatedStandardsSearch.toLowerCase()))
                      .map(s => {
                        const currentRelated = editingStandard.relatedStandardIds || [];
                        const isChecked = currentRelated.includes(s.id);
                        return (
                          <label key={s.id} className="flex items-center gap-2 p-1.5 hover:bg-white rounded transition-colors text-sm text-gray-700 cursor-pointer">
                            <input
                              type="checkbox"
                              checked={isChecked}
                              onChange={() => {
                                let newRelated = [...currentRelated];
                                if (isChecked) {
                                  newRelated = newRelated.filter(id => id !== s.id);
                                } else {
                                  newRelated.push(s.id);
                                }
                                setEditingStandard({
                                  ...editingStandard,
                                  relatedStandardIds: newRelated
                                });
                              }}
                              className="rounded text-blue-600 focus:ring-blue-500"
                            />
                    ...
                    {standards.filter(s => s.id !== editingStandard.id && s.name.toLowerCase().includes(relatedStandardsSearch.toLowerCase())).length === 0 && (
                      <p className="text-center text-xs text-gray-500 py-3">No hay otros estándares disponibles.</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
              <button
                ...
                className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-50 transition cursor-pointer"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition shadow-sm flex items-center gap-1.5 cursor-pointer"
              >
                Guardar Estándar
              </button>
            </div>
